...
class GraphICA(nn.Module):
    def __init__(self, in_dim, hid_dims, out_dim, use_mlp = False,
        ...
        self.decode = LogReg(out_dim, 2)

    def get_embedding(self, data1):
        if self.use_mlp:
            h1 = self.backbone_mlp(data1.x)
        else:
            h1 = data1.x
        if self.use_graph:
            h1 = self.backbone_gcn(h1, data1.edge_index)
        if self.regularize:
            out = (h1 - h1.mean(0)) / h1.std(0)
        else:
            out = h1
        return out.detach()
...
class iVGAE(nn.Module):
    ...
    def encoder_params(self, x, u, edge_index):
        xu = torch.cat((x, u), 1)
        g = self.g(xu, edge_index)
        logv = self.logv(xu, edge_index)
        return g, logv.exp()
    ...
    def forward(self, x, u, edge_index):
        ...
        encoder_params = self.encoder_params(x, u, edge_index)
    ...
    def elbo(self, x, u, edge_index):
        decoder_params, (g, v), z, prior_params = self.forward(x, u, edge_index)
